...
/**
 * One-time migration route to create Creative Hub tables in Supabase.
 * Call: POST /api/admin/migrate-creative-hub
...
export async function POST() {
  ...
  for (const sql of statements) {
    try {
      ...
      if (!res.ok) {
        // Try the SQL API directly (Supabase Management API)
        ...
        if (!pgRes.ok) {
          ...
          results.push({ sql: sql.substring(0, 60) + '...', ok: false, error: errText.substring(0, 200) });
          continue;
        }
      }
  ...
  return NextResponse.json({
    success: allOk,
    results,
    message: allOk ? 'All tables created successfully' : 'Some statements failed — check results'
  });
}
