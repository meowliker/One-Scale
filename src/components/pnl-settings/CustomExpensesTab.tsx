...
// Legacy props interface kept for backwards compatibility — no longer used
...
/**
 * Custom Expenses tab for P&L Settings.
 * Uses /api/pnl/expenses (Supabase direct) — same data source as the
 * standalone expenses page. Dark-mode themed with CSV import support.
 */
export function CustomExpensesTab(_props: CustomExpensesTabProps) {
  ...
  function openEdit(exp: DbExpense) {
    setEditingExpense({
      id: exp.id, name: exp.name, category: exp.category, amount: exp.amount,
      frequency: exp.frequency, distribution: exp.distribution as 'daily' | 'hourly' | 'smart',
      startDate: exp.start_date, endDate: exp.end_date, isActive: exp.is_active,
    });
    setPanelOpen(true);
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-text-secondary">Track fixed and variable costs that affect your P&L</p>
        <div className="flex items-center gap-2.5">
          <button
      ...
      {/* Expense Panel (slide-over form with impact preview) */}
      <ExpensePanel
        open={panelOpen}
